...
class GridTests(unittest.TestCase):
    ...
    def test_color_of_Coordinate(self):
        ...
        grid.grid[coord[1] * self.unit_size, coord[0] * self.unit_size, :] = (
            expected_color
        )
    ...
    def test_draw_Positive(self):
        ...
        for y in range(grid.grid.shape[0]):
            for x in range(grid.grid.shape[1]):
                if (
                    y >= coord[1] * self.unit_size
                    and y < coord[1] * self.unit_size + grid.unit_size - grid.unit_gap
                    and x >= coord[0] * self.unit_size
                    and x < coord[0] * self.unit_size + grid.unit_size - grid.unit_gap
                ):
    ...
    def test_draw_snake_Positive(self):
        ...
        expected_colors = np.array(
            [grid.HEAD_COLOR, grid.BODY_COLOR, grid.BODY_COLOR], dtype=np.uint8
        )
    ...
    def test_draw_snake_Negative(self):
        ...
        for i, j in zip(range(grid.grid_size[0]), range(grid.grid_size[1])):
            coord = (i, j)
            if (
                coord == expected_coords[0]
                or coord == expected_coords[1]
                or coord == expected_coords[2]
            ):
    ...
    def test_connect_x(self):
        ...
        for y in range(grid.grid.shape[0]):
            for x in range(grid.grid.shape[1]):
                if (
                    y == coord1[1] * self.unit_size
                    or y
                    == coord1[1] * self.unit_size + grid.unit_size - grid.unit_gap - 1
                ) and (
                    x < coord2[0] * self.unit_size
                    and x >= coord1[0] * self.unit_size + grid.unit_size - grid.unit_gap
                ):
                    self.assertTrue(np.array_equal(grid.grid[y, x, :], expected_color))
                else:
                    self.assertFalse(np.array_equal(grid.grid[y, x, :], expected_color))
    ...
    def test_connect_y(self):
        ...
        for y in range(grid.grid.shape[0]):
            for x in range(grid.grid.shape[1]):
                if (
                    x == coord1[0] * self.unit_size
                    or x
                    == coord1[0] * self.unit_size + grid.unit_size - grid.unit_gap - 1
                ) and (
                    y < coord2[1] * self.unit_size
                    and y >= coord1[1] * self.unit_size + grid.unit_size - grid.unit_gap
                ):
                    self.assertTrue(np.array_equal(grid.grid[y, x, :], expected_color))
                else:
                    self.assertFalse(np.array_equal(grid.grid[y, x, :], expected_color))
        ...
        for y in range(grid.grid.shape[0]):
            for x in range(grid.grid.shape[1]):
                if (
                    y >= coord1[1] * self.unit_size
                    and y < coord1[1] * self.unit_size + grid.unit_size - grid.unit_gap
                    and x >= coord1[0] * self.unit_size
                    and x < coord1[0] * self.unit_size + grid.unit_size - grid.unit_gap
                ):
                    self.assertTrue(np.array_equal(grid.grid[y, x, :], grid.BODY_COLOR))
                else:
                    self.assertFalse(
                        np.array_equal(grid.grid[y, x, :], grid.BODY_COLOR)
                    )
